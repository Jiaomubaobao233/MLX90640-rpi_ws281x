...
import sys
import random
#sys.path.append('/home/pi/rpi_ws281x/python/build/lib.linux-armv71-2.7/')
from neopixel import *
#import neopixel

# RUN USE:
# sudo PYTHONPATH=".:build/lib.linux-armv71-2.7" python3 examples/test.py

# LED strip configuration:
LED_COUNT_18   = 100 * 7  # Number of LED pixels.
LED_PIN_18     = 18      # GPIO pin connected to the pixels (18 uses PWM!).
LED_DMA_18     = 10      # DMA channel to use for generating signal (try 10)

LED_COUNT_17   = 60 * 5  # Number of LED pixels.
LED_PIN_17     = 17      # GPIO pin connected to the pixels (18 uses PWM!).
LED_DMA_17     = 11      # DMA channel to use for generating signal (try 10)

LED_COUNT_27   = 60 * 5  # Number of LED pixels.
LED_PIN_27     = 27      # GPIO pin connected to the pixels (18 uses PWM!).
LED_DMA_27     = 12      # DMA channel to use for generating signal (try 10)

LED_COUNT_22   = 60 * 5  # Number of LED pixels.
LED_PIN_22     = 22      # GPIO pin connected to the pixels (18 uses PWM!).
LED_DMA_22     = 13      # DMA channel to use for generating signal (try 10)

LED_COUNT_23   = 60 * 5  # Number of LED pixels.
LED_PIN_23     = 23      # GPIO pin connected to the pixels (18 uses PWM!).
LED_DMA_23     = 9      # DMA channel to use for generating signal (try 10)

LED_COUNT_24   = 60 * 5  # Number of LED pixels.
LED_PIN_24     = 24      # GPIO pin connected to the pixels (18 uses PWM!).
LED_DMA_24     = 8      # DMA channel to use for generating signal (try 10)

LED_COUNT_25   = 60 * 5  # Number of LED pixels.
LED_PIN_25     = 25      # GPIO pin connected to the pixels (18 uses PWM!).
LED_DMA_25     = 7      # DMA channel to use for generating signal (try 10)



#LED_PIN        = 10      # GPIO pin connected to the pixels (10 uses SPI /dev/spidev0.0).
LED_FREQ_HZ    = 800000  # LED signal frequency in hertz (usually 800khz)
LED_BRIGHTNESS = 15 # 255     # Set to 0 for darkest and 255 for brightest
...
class MLX90640:
        ...
        while(True):
            ...
            for y in range(24):
                for x in range(32): 
                    ...
                    val3 = (val1+val2)/2
                    '''if ((x+y)<5)|((31-x+y)<5)|((31-x+23-y)<5)|((x+23-y)<5):
                        val3 = 0
                    print('%4.1f ' % val3, end='')'''
                    image[y][x] = val3
                    if val3 > maxt:
            ...
            #set thresholdt = maxt at first frame + 1
            '''if thresholdt == 0:
                thresholdt = maxt + 1'''
            thresholdt=24

            #if maxt < thresholdt, then there's no one here
            if maxt > thresholdt:
                for y in range(24):
                    for x in range(32): 
                        val3 = image[y][x] + 3 - maxt
                        if val3 < 0: continue
                        val3 = val3 * val3 * val3
                        sumy = sumy + y * val3
                        sumx = sumx + x * val3
            ...
            #image[round(avey)][round(avex)] = -10
            imagef = cv2.GaussianBlur(image, (5,5), 0)#5,5
    ...
    #         print(image_color.shape)
            ...
            print('MLX90640 Thermal Imagery (%7.3f,%7.3f), maxt=%5.3f, thresholdt=%5.3f' % (avey, avex, maxt, thresholdt))
            if avex==404:
                avex=0
            if avex>0:
                if avex>6:
...
def strandZYF(strip):
    ...
    global glox, gloy
    global color1, color2, color3 #r, g, b
    if __name__ == '__main__':
        # Process arguments7-[p]
       ...
       #print ('Press Ctrl-C to quit.')
        ...
        try:
            while True:
                ...
                #theaterChase(str18, Color(127, 127, 127))  # White theater chase
                print('(glox,gloy)=(%7.3f,%7.3f)' % (glox,gloy))
...
global glox, gloy
global color1, color2, color3 #R B G
glox = 0
gloy = 0
color1 = 127
color2 = 127
...
str18 = Adafruit_NeoPixel(LED_COUNT_18, LED_PIN_18, LED_FREQ_HZ, LED_DMA_18, LED_INVERT, LED_BRIGHTNESS, LED_CHANNEL_0)
'''str23 = Adafruit_NeoPixel(LED_COUNT_23, LED_PIN_23, LED_FREQ_HZ, LED_DMA_23, LED_INVERT, LED_BRIGHTNESS, LED_CHANNEL_0)
str24 = Adafruit_NeoPixel(LED_COUNT_24, LED_PIN_24, LED_FREQ_HZ, LED_DMA_24, LED_INVERT, LED_BRIGHTNESS, LED_CHANNEL_0)
str25 = Adafruit_NeoPixel(LED_COUNT_25, LED_PIN_25, LED_FREQ_HZ, LED_DMA_25, LED_INVERT, LED_BRIGHTNESS, LED_CHANNEL_0)
str17 = Adafruit_NeoPixel(LED_COUNT_17, LED_PIN_17, LED_FREQ_HZ, LED_DMA_17, LED_INVERT, LED_BRIGHTNESS, LED_CHANNEL_0)
str27 = Adafruit_NeoPixel(LED_COUNT_27, LED_PIN_27, LED_FREQ_HZ, LED_DMA_27, LED_INVERT, LED_BRIGHTNESS, LED_CHANNEL_0)
str22 = Adafruit_NeoPixel(LED_COUNT_22, LED_PIN_22, LED_FREQ_HZ, LED_DMA_22, LED_INVERT, LED_BRIGHTNESS, LED_CHANNEL_0)'''
thermalcamera = threading.Thread(target=thermalcameraZYF)
thermalcamera.start()

strand18 = threading.Thread(target=strandZYF(str18))
'''strand23 = threading.Thread(target=strandZYF(str23))
strand24 = threading.Thread(target=strandZYF(str24))
strand25 = threading.Thread(target=strandZYF(str25))
strand17 = threading.Thread(target=strandZYF(str17))
strand27 = threading.Thread(target=strandZYF(str27))
strand22 = threading.Thread(target=strandZYF(str22))'''
